...
function addListenersToDataChannel(callerID) {
  var ID = callerID.ID;
  dataChannels[ID].channel.onerror = utils.errorHandler;
...
//for testing
domainHandler.differenceInDomain = differenceInDomain;
...
function getMyID() {
  if (myID) {
    ...
    utils.printDebugMessage('myID has not been defined');
  }
}

function isForMe(callerID) {
  if (myID && callerID.ID) {
    return myID.ID === callerID.ID;
  }
  if (myID && callerID) {
    return myID.ID === callerID;
  }
  return false;
}
...
function differenceInDomain(event, oldDomain) {
  ...
  for (var room in largerDomain) {
    if (differenceInRooms(smallerDomain[room], largerDomain[room], room)) {
      return differenceInRooms(smallerDomain[room], largerDomain[room], room);
    }
  }
}

function differenceInRooms(smallerRoom, largerRoom, room) {
  var options = {};
  if (!smallerRoom) {
    options.room = room;
    options.callerID = largerRoom[0].ID;
    options.ID = largerRoom[0].ID;
    options.joined_at = largerRoom[0].joined_at;
    return options;
  }

  if (smallerRoom.length !== largerRoom.length) {
    for (var i = 0; i < largerRoom.length; i++) {
      if (!_.findWhere(smallerRoom, largerRoom[i])) {
        options.room = room;
        options.callerID = largerRoom[i].ID;
        options.ID = largerRoom[i].ID;
        options.joined_at = largerRoom[i].joined_at;
        return options;
      }
    }
  }
...
function call(callerID) {
  var isCaller = true;
  createPeerConnection(callerID, isCaller);

  if (r.wantsStream()) {
    addLocalStream(callerID.ID);
  }
...
function createPeerConnection(callerID, isCaller) {
  ...
  localPeerConnections[ID].channel.onnegotiationneeded = utils.negotiationNeededHandler;
...
function createOffer(callerID) {
  var ID = callerID.ID;
  localPeerConnections[ID].channel.createOffer(function(description) {
...
function createDescription(description, callerID, offerOrAnswer) {
  ...
  localPeerConnections[ID].channel.setLocalDescription(description, function() {
    utils.printDebugMessage('sending ' + offerOrAnswer + ' to ' + ID);
...
function offerEvent(callerID, remoteDescription) {
  ...
  setRemoteDescription(ID, offer, function() {
    createAnswer(callerID);
  });
}

function setRemoteDescription(ID, description, callback) {
  localPeerConnections[ID].channel.setRemoteDescription(description, callback, utils.errorHandler);
}

function createAnswer(callerID) {
  var ID = callerID.ID;
  localPeerConnections[ID].channel.createAnswer(function(localDescription) {
...
function answerEvent(callerID, remoteDescription) {
  var ID = callerID.ID;
  utils.printDebugMessage('receive answer from ' + ID);
  var answer = new RTCSessionDescription(remoteDescription);
  setRemoteDescription(ID, answer, function() {
...
function addLocalStream(ID) {
  localPeerConnections[ID].channel.addStream(s.getLocalStream());
}
...
var d = require('./domainHandler');
var b = require('./browserHandler');
var StreamOptions = require('./streamOptionsModel');
...
streamHandler.removeRemoteVideo = removeRemoteVideo;
...
function removeRemoteVideo(ID) {
  if (remoteVideos[ID]) {
    remoteVideos[ID].src = null;
    delete remoteVideos[ID];
  }
}

StreamOptions.prototype.getVideo = function() {
  var tempVideo = utils.createVideoElement(this.stream, this.source, this.callerID);
  remoteVideos[this.ID] = tempVideo;
  return tempVideo;
}
...
function createRoomSettings(settings) {
  ...
  if (settings && settings.limit && settings.limit > 0) {
